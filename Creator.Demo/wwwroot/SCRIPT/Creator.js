...
const capitalize = (str) => upper(str.charAt(0)) + str.slice(1);
...
const addClass = (type, elm, className) => get(type, elm).classList.add(className);

// removes a name from the class attribute of an element
const removeClass = (type, elm, className) => get(type, elm).classList.remove(className);
